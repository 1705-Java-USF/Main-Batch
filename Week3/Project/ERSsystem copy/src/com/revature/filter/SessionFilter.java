...
public class SessionFilter implements Filter {
	...
	public void doFilter(ServletRequest req, ServletResponse res, FilterChain chain)
		...
		} else if (session == null || session.getAttribute("ERS_USERS") == null) {
			response.sendRedirect("index.jsp");
		} else if (manList.contains(url) && ((ERS_USERS) request.getSession().getAttribute("ERS_USERS")).getRole().equals("Manager")) {
			chain.doFilter(req, res);
		} else if (empList.contains(url)
				&& ((ERS_USERS) request.getSession().getAttribute("ERS_USERS")).getRole().equals("Employee")) {
			chain.doFilter(req, res);

		} else {
			if (((ERS_USERS) request.getSession().getAttribute("ERS_USERS")).getRole().equals("Manager")) {
				response.sendRedirect("ManReimbursement.jsp");
			} else if (((ERS_USERS) request.getSession().getAttribute("ERS_USERS")).getRole().equals("Employee")) {
